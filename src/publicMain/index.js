...
import { AVATAR_STORE_PATH, EDITOR_PGAE, GRAPH_PNG_STORE_PATH } from "../public/js/urls";
...
window.addEventListener("load", async () => {
    ...
    if (graphListResponse.state == 1 && userListResponse.state == 1) {
        ...
        graphList.forEach(graph => {
            ...
            domGraphBlockText.innerHTML = `
            <img src="${AVATAR_STORE_PATH + graph.author.img}"><span>${graph.name.length < 6 ? graph.name : graph.name.slice(0, 5) + "..."}</span>
            `;
